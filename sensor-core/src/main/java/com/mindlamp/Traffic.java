...
import com.mindlamp.providers.Traffic_Provider.Traffic_Data;
 ...
 * @author denzil
 */
public class Traffic extends Lamp_Sensor {

    /**
     * Logging tag (default = "LAMP::Network traffic")
     */
    public static String TAG = "LAMP::Network traffic";

    /**
     * Broadcasted event: updated traffic information is available
     */
    public static final String ACTION_LAMP_NETWORK_TRAFFIC = "ACTION_LAMP_NETWORK_TRAFFIC";
    ...
    private final Runnable mRunnable = new Runnable() {
        ...
        public void run() {
            ...
            wifi.put(Traffic_Data.DEVICE_ID, Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEVICE_ID));
            ...
            if (Lamp.DEBUG) Log.d(TAG, "Wifi:" + wifi.toString());

            ContentValues network = new ContentValues();
            network.put(Traffic_Data.TIMESTAMP, System.currentTimeMillis());
            network.put(Traffic_Data.DEVICE_ID, Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEVICE_ID));
            ...
            if (Lamp.DEBUG) Log.d(TAG, "Network: " + network.toString());

            Intent traffic = new Intent(ACTION_LAMP_NETWORK_TRAFFIC);
    ...
    private static Traffic.LAMPSensorObserver awareSensor;

    public static void setSensorObserver(Traffic.LAMPSensorObserver observer) {
        awareSensor = observer;
    }

    public static Traffic.LAMPSensorObserver getSensorObserver() {
        return awareSensor;
    }
    ...
    public void onCreate() {
        ...
        if (Lamp.DEBUG) Log.d(TAG, "Traffic service created!");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (PERMISSIONS_OK) {
            ...
            if (startTotalRxBytes == TrafficStats.UNSUPPORTED) {
                Lamp.setSetting(getApplicationContext(), Lamp_Preferences.STATUS_NETWORK_TRAFFIC, false);
                if (Lamp.DEBUG)
                    Log.d(TAG, "Device doesn't support traffic statistics! Disabling sensor...");
                Lamp.stopTraffic(this);

            } else {

                DEBUG = Lamp.getSetting(this, Lamp_Preferences.DEBUG_FLAG).equals("true");
                Lamp.setSetting(this, Lamp_Preferences.STATUS_NETWORK_TRAFFIC, true);
                ...
                if (Lamp.DEBUG) Log.d(TAG, "Traffic service active...");

                if (Lamp.isStudy(this)) {
                    ContentResolver.setIsSyncable(Lamp.getLAMPAccount(this), Traffic_Provider.getAuthority(this), 1);
                    ContentResolver.setSyncAutomatically(Lamp.getLAMPAccount(this), Traffic_Provider.getAuthority(this), true);
                    long frequency = Long.parseLong(Lamp.getSetting(this, Lamp_Preferences.FREQUENCY_WEBSERVICE)) * 60;
                    SyncRequest request = new SyncRequest.Builder()
                            .syncPeriodic(frequency, frequency / 3)
                            .setSyncAdapter(Lamp.getLAMPAccount(this), Traffic_Provider.getAuthority(this))
    ...
    public void onDestroy() {
        ...
        ContentResolver.setSyncAutomatically(Lamp.getLAMPAccount(this), Traffic_Provider.getAuthority(this), false);
        ...
        if (Lamp.DEBUG) Log.d(TAG, "Traffic service terminated...");
    }
}
