...
public class ESM_Question extends DialogFragment {
    ...
    public class ESMExpireMonitor extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
                    ...
                    if (esm != null && esm.moveToFirst()) {
                        ...
                        switch (status) {
                            case ESM.STATUS_ANSWERED:
                                if (Lamp.DEBUG) Log.d(Lamp.TAG, "ESM has been answered!");
                                break;
                            case ESM.STATUS_DISMISSED:
                                if (Lamp.DEBUG) Log.d(Lamp.TAG, "ESM has been dismissed!");
                                break;
                        }
                    }
            ...
            if (Lamp.DEBUG) Log.d(Lamp.TAG, "ESM has expired!");
    ...
    private void cancelESM() {
        ...
        if (pendingESM != null && pendingESM.moveToFirst()) {
            if (Lamp.DEBUG) Log.d(Lamp.TAG, "Rest of ESM Queue is dismissed!");
            do {
        ...
        Intent answer = new Intent(ESM.ACTION_LAMP_ESM_DISMISSED);
    ...
    public void onPause() {
        ...
        if (ESM.isESMVisible(getActivity().getApplicationContext())) {
            if (Lamp.DEBUG)
                Log.d(Lamp.TAG, "ESM was visible but not answered, go back to notification bar");
