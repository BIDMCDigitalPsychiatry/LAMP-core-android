...
import com.mindlamp.utils.Http;
import com.mindlamp.utils.Https;
import com.mindlamp.utils.SSLManager;
 ...
 */
public class LampSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public LampSyncAdapter(Context context, boolean autoInitialize, boolean allowParallelSyncs) {
        ...
        dontClearSensors.add("mindlamp_studies");
    }

    /**
     * Sends the data to LAMP server of the study
     *
     * @param account
     * @param extras
     * @param authority
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        if (!Lamp.getSetting(mContext, Lamp_Preferences.WEBSERVICE_SILENT).equals("true"))
        ...
        if (DATABASE_TABLES != null && TABLES_FIELDS != null && CONTEXT_URIS != null) {
            for (int i = 0; i < DATABASE_TABLES.length; i++) {
                offloadData(mContext, DATABASE_TABLES[i], Lamp.getSetting(getContext(), Lamp_Preferences.WEBSERVICE_SERVER), TABLES_FIELDS[i], CONTEXT_URIS[i]);
            }
        }
    }
    ...
    private void offloadData(Context context, String database_table, String web_server, String table_fields, Uri CONTENT_URI) {
        ...
        if (Lamp.getSetting(context, Lamp_Preferences.WEBSERVICE_CHARGING).equals("true")) {
            ...
            if (!isCharging) {
                if (Lamp.DEBUG) Log.d(Lamp.TAG, "Only sync data if charging...");
                return;
            }
        }
        ...
        if (!isWifiNeededAndConnected()) {
            if (!isForce3G(database_table)) {
                if (Lamp.DEBUG)
                    Log.d(Lamp.TAG, "Sync data only over Wi-Fi. Will try again later...");
                return;
            }
        }

        Lamp.debug(mContext, "STUDY-SYNC: " + database_table);

        String protocol = web_server.substring(0, web_server.indexOf(":"));
        boolean web_service_simple = Lamp.getSetting(context, Lamp_Preferences.WEBSERVICE_SIMPLE).equals("true");
        boolean web_service_remove_data = Lamp.getSetting(context, Lamp_Preferences.WEBSERVICE_REMOVE_DATA).equals("true");
        ...
        if (MAX_POST_SIZE == 0) {
            Log.d(Lamp.TAG, "Device without available memory left for sync.");
            return;
        }

        if (Lamp.is_watch(context)) {
            MAX_POST_SIZE = 100; //default for Android Wear (we have a limit of 100KB of data packet size (Message API restrictions)
        }

        if (Lamp.DEBUG)
            Log.d(Lamp.TAG, "Syncing " + database_table + " to: " + web_server + " in batches of " + MAX_POST_SIZE);

        String device_id = Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID);
        boolean DEBUG = Lamp.getSetting(context, Lamp_Preferences.DEBUG_FLAG).equals("true");
        ...
        if (response != null || web_service_simple) {
            try {
                ...
                if (total_records > 0) {
                    ...
                    do {
                        if (!Lamp.getSetting(context, Lamp_Preferences.WEBSERVICE_SILENT).equals("true"))
                        ...
                        if (lastSynced == null) {
                            removeFrom = 0;
                            Log.d(Lamp.TAG, "Connection to server interrupted. Will try again later.");
                            break;
                        } else {
                    ...
                    if (DEBUG)
                        Log.d(Lamp.TAG, database_table + " sync time: " + DateUtils.formatElapsedTime((System.currentTimeMillis() - start) / 1000));
    ...
    private void notifyUser(Context mContext, String message, boolean dismiss, boolean indetermined, int id) {
        if (!dismiss) {
            NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(mContext, Lamp.LAMP_NOTIFICATION_CHANNEL_DATASYNC);
            ...
            mBuilder = Lamp.setNotificationProperties(mBuilder, Lamp.LAMP_NOTIFICATION_IMPORTANCE_DATASYNC);
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                mBuilder.setChannelId(Lamp.LAMP_NOTIFICATION_CHANNEL_DATASYNC);

            try {
                notManager.notify(id, mBuilder.build());
            } catch (NullPointerException e) {
                if (Lamp.DEBUG) Log.d(Lamp.TAG, "Notification exception: " + e);
            }
        } else {
            try {
    ...
    public boolean isWifiNeededAndConnected() {
        if (Lamp.getSetting(mContext, Lamp_Preferences.WEBSERVICE_WIFI_ONLY).equals("true")) {
    ...
    public boolean isForce3G(String database_table) {
        if (Lamp.getSetting(mContext, Lamp_Preferences.WEBSERVICE_FALLBACK_NETWORK).length() > 0 && !Lamp.getSetting(mContext, Lamp_Preferences.WEBSERVICE_FALLBACK_NETWORK).equals("0")) {
            Cursor lastSynched = mContext.getContentResolver().query(Lamp_Provider.Aware_Log.CONTENT_URI, null, Lamp_Provider.Aware_Log.LOG_MESSAGE + " LIKE 'STUDY-SYNC: " + database_table + "'", null, Lamp_Provider.Aware_Log.LOG_TIMESTAMP + " DESC LIMIT 1");
            if (lastSynched != null && lastSynched.moveToFirst()) {
                long synched = lastSynched.getLong(lastSynched.getColumnIndex(Lamp_Provider.Aware_Log.LOG_TIMESTAMP));

                Log.d(Lamp.TAG, "Checking forced sync over 3G...");
                Log.d(Lamp.TAG, "Last sync: " + synched + " elapsed: " + (System.currentTimeMillis() - synched) + " force: " + (System.currentTimeMillis() - synched >= Integer.parseInt(Lamp.getSetting(mContext, Lamp_Preferences.WEBSERVICE_FALLBACK_NETWORK)) * 60 * 60 * 1000));

                lastSynched.close();
                return (System.currentTimeMillis() - synched >= Integer.parseInt(Lamp.getSetting(mContext, Lamp_Preferences.WEBSERVICE_FALLBACK_NETWORK)) * 60 * 60 * 1000);
            } else
    ...
    private String createRemoteTable(String DEVICE_ID, String TABLES_FIELDS, Boolean WEBSERVICE_SIMPLE, String protocol, Context mContext, String WEBSERVER, String DATABASE_TABLE) {
        ...
        fields.put(Lamp_Preferences.DEVICE_ID, DEVICE_ID);
    ...
    private String getLatestRecordSynced(String database_table, String[] columnsStr) {
        ...
        Cursor lastSynced = mContext.getContentResolver().query(Lamp_Provider.Aware_Log.CONTENT_URI, null, Lamp_Provider.Aware_Log.LOG_MESSAGE + " LIKE '{\"table\":\"" + database_table + "\",\"last_sync_timestamp\":%'", null, Lamp_Provider.Aware_Log.LOG_TIMESTAMP + " DESC LIMIT 1");
        if (lastSynced != null && lastSynced.moveToFirst()) {
            try {

                JSONObject logSyncData = new JSONObject(lastSynced.getString(lastSynced.getColumnIndex(Lamp_Provider.Aware_Log.LOG_MESSAGE)));
    ...
    private String getLatestRecordFromServer(String DEVICE_ID, Boolean WEBSERVICE_SIMPLE, Boolean WEBSERVICE_REMOVE_DATA, String DATABASE_TABLE, String protocol, Context mContext, String WEBSERVER) {
        ...
        request.put(Lamp_Preferences.DEVICE_ID, DEVICE_ID);
        ...
        if (!(WEBSERVICE_SIMPLE && WEBSERVICE_REMOVE_DATA) || dontClearSensors.contains(DATABASE_TABLE)) {
            // Normal LAMP API always gets here.
            if (protocol.equals("https")) {
                try {
    ...
    private String getStudySyncCondition(Context mContext, String DATABASE_TABLE) {
        ...
        if (Lamp.isStudy(mContext)) {
            Cursor study = Lamp.getStudy(mContext, Lamp.getSetting(mContext, Lamp_Preferences.WEBSERVICE_SERVER));
            if (study != null && study.moveToFirst()) {
                study_condition += " AND timestamp >= " + study.getLong(study.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_JOINED));
            }
    ...
    private void performDatabaseSpaceMaintenance(Uri CONTENT_URI, long last, String[] columnsStr, Boolean WEBSERVICE_REMOVE_DATA, Context mContext, String DATABASE_TABLE, Boolean DEBUG) {
        ...
        if (WEBSERVICE_REMOVE_DATA) {
        ...
        } else if (Lamp.getSetting(mContext, Lamp_Preferences.FREQUENCY_CLEAN_OLD_DATA).length() > 0) {
            Calendar cal = Calendar.getInstance();
            cal.setTimeInMillis(last);
            int rowsDeleted = 0;
            switch (Integer.parseInt(Lamp.getSetting(mContext, Lamp_Preferences.FREQUENCY_CLEAN_OLD_DATA))) {
                case 1: //Weekly
                    cal.add(Calendar.DAY_OF_YEAR, -7);
                    if (Lamp.DEBUG)
                        Log.d(Lamp.TAG, " Cleaning locally any data older than last week (yyyy/mm/dd): " + cal.get(Calendar.YEAR) + '/' + (cal.get(Calendar.MONTH) + 1) + '/' + cal.get(Calendar.DAY_OF_MONTH));
                ...
                case 2: //Monthly
                    ...
                    if (Lamp.DEBUG)
                        Log.d(Lamp.TAG, " Cleaning locally any data older than last month (yyyy/mm/dd): " + cal.get(Calendar.YEAR) + '/' + (cal.get(Calendar.MONTH) + 1) + '/' + cal.get(Calendar.DAY_OF_MONTH));
                    rowsDeleted = mContext.getContentResolver().delete(CONTENT_URI, "timestamp < " + cal.getTimeInMillis() + deleteSessionBasedSensors, null);
                    break;
                case 3: //Daily
                    cal.add(Calendar.DAY_OF_YEAR, -1);
                    if (Lamp.DEBUG)
                        Log.d(Lamp.TAG, "Cleaning locally any data older than today (yyyy/mm/dd): " + cal.get(Calendar.YEAR) + '/' + (cal.get(Calendar.MONTH) + 1) + '/' + cal.get(Calendar.DAY_OF_MONTH) + " from " + CONTENT_URI.toString());
                    rowsDeleted = mContext.getContentResolver().delete(CONTENT_URI, "timestamp < " + cal.getTimeInMillis() + deleteSessionBasedSensors, null);
                    break;
            ...
            if (DEBUG && rowsDeleted > 0)
                Log.d(Lamp.TAG, "Cleaned " + rowsDeleted + " from " + CONTENT_URI.toString());
        }
    }
    ...
    private Long syncBatch(Cursor context_data, String DATABASE_TABLE, String DEVICE_ID, Context mContext, String protocol, String WEBSERVER, Boolean DEBUG) throws JSONException {
        ...
        if (context_data != null && context_data.moveToFirst()) {
            ...
            if (success == null) {
                if (DEBUG) Log.d(Lamp.TAG, DATABASE_TABLE + " FAILED to sync. Server down?");
                return null;
            } else {

                try {
                    Lamp.debug(mContext, new JSONObject()
                ...
                if (DEBUG)
                    Log.d(Lamp.TAG, "Sync OK into " + DATABASE_TABLE + " [ " + rows.length() + " rows ]");
            }
        }
