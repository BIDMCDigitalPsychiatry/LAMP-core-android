...
import com.mindlamp.providers.Battery_Provider.Battery_Charges;
import com.mindlamp.providers.Battery_Provider.Battery_Data;
import com.mindlamp.providers.Battery_Provider.Battery_Discharges;
 ...
 * @author denzil
 */
public class Battery extends Lamp_Sensor {

    /**
     * Logging tag (default = "LAMP::Service")
     */
    public static String TAG = "LAMP::Battery";

    /**
     * Broadcasted event: the battery values just changed
     */
    public static final String ACTION_LAMP_BATTERY_CHANGED = "ACTION_LAMP_BATTERY_CHANGED";

    /**
     * Broadcasted event: the user just started charging
     */
    public static final String ACTION_LAMP_BATTERY_CHARGING = "ACTION_LAMP_BATTERY_CHARGING";

    /**
     * Broadcasted event: battery charging over power supply (AC)
     */
    public static final String ACTION_LAMP_BATTERY_CHARGING_AC = "ACTION_LAMP_BATTERY_CHARGING_AC";

    /**
     * Broadcasted event: battery charging over USB
     */
    public static final String ACTION_LAMP_BATTERY_CHARGING_USB = "ACTION_LAMP_BATTERY_CHARGING_USB";

    /**
     * Broadcasted event: the user just stopped charging and is running on battery
     */
    public static final String ACTION_LAMP_BATTERY_DISCHARGING = "ACTION_LAMP_BATTERY_DISCHARGING";

    /**
     * Broadcasted event: the battery is fully charged
     */
    public static final String ACTION_LAMP_BATTERY_FULL = "ACTION_LAMP_BATTERY_FULL";

    /**
     * Broadcasted event: the battery is running low and should be charged ASAP
     */
    public static final String ACTION_LAMP_BATTERY_LOW = "ACTION_LAMP_BATTERY_LOW";

    /**
     * Broadcasted event: the phone is about to be shutdown.
     */
    public static final String ACTION_LAMP_PHONE_SHUTDOWN = "ACTION_LAMP_PHONE_SHUTDOWN";

    /**
     * Broadcasted event: the phone is about to be rebooted.
     */
    public static final String ACTION_LAMP_PHONE_REBOOT = "ACTION_LAMP_PHONE_REBOOT";
    ...
    public class Battery_Broadcaster extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(Intent.ACTION_BATTERY_CHANGED)) {
                ...
                rowData.put(Battery_Data.DEVICE_ID, Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID));
                ...
                if (extras.getInt(BatteryManager.EXTRA_PLUGGED) == BatteryManager.BATTERY_PLUGGED_AC) {
                    if (Lamp.DEBUG) Log.d(TAG, ACTION_LAMP_BATTERY_CHARGING_AC);
                    Intent battChargeAC = new Intent(ACTION_LAMP_BATTERY_CHARGING_AC);
                    context.sendBroadcast(battChargeAC);
                }

                if (extras.getInt(BatteryManager.EXTRA_PLUGGED) == BatteryManager.BATTERY_PLUGGED_USB) {
                    if (Lamp.DEBUG) Log.d(TAG, ACTION_LAMP_BATTERY_CHARGING_USB);
                    Intent battChargeUSB = new Intent(ACTION_LAMP_BATTERY_CHARGING_USB);
                    context.sendBroadcast(battChargeUSB);
                }

                if (extras.getInt(BatteryManager.EXTRA_STATUS) == BatteryManager.BATTERY_STATUS_FULL) {
                    if (Lamp.DEBUG) Log.d(TAG, ACTION_LAMP_BATTERY_FULL);
                    Intent battFull = new Intent(ACTION_LAMP_BATTERY_FULL);
                    context.sendBroadcast(battFull);
                }

                if (Lamp.DEBUG) Log.d(TAG, ACTION_LAMP_BATTERY_CHANGED);
                Intent battChanged = new Intent(ACTION_LAMP_BATTERY_CHANGED);
            ...
            if (intent.getAction().equals(Intent.ACTION_POWER_CONNECTED)) {
                ...
                if (lastBattery != null && lastBattery.moveToFirst()) {
                    ...
                    rowData.put(Battery_Charges.DEVICE_ID, Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID));
                ...
                if (Lamp.DEBUG) Log.d(TAG, ACTION_LAMP_BATTERY_CHARGING);
                Intent battChanged = new Intent(ACTION_LAMP_BATTERY_CHARGING);
                context.sendBroadcast(battChanged);
            ...
            if (intent.getAction().equals(Intent.ACTION_POWER_DISCONNECTED)) {
                ...
                if (lastBattery != null && lastBattery.moveToFirst()) {
                    ...
                    rowData.put(Battery_Discharges.DEVICE_ID, Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID));
                ...
                if (Lamp.DEBUG) Log.d(TAG, ACTION_LAMP_BATTERY_DISCHARGING);
                Intent battChanged = new Intent(ACTION_LAMP_BATTERY_DISCHARGING);
                context.sendBroadcast(battChanged);
            ...
            if (intent.getAction().equals(Intent.ACTION_BATTERY_LOW)) {
                if (Lamp.DEBUG) Log.d(TAG, ACTION_LAMP_BATTERY_LOW);

                if (awareSensor != null) awareSensor.onBatteryLow();

                Intent battChanged = new Intent(ACTION_LAMP_BATTERY_LOW);
                context.sendBroadcast(battChanged);
            }
            ...
            if (intent.getAction().equals(Intent.ACTION_SHUTDOWN)) {
                ...
                if (Lamp.DEBUG) Log.d(TAG, ACTION_LAMP_PHONE_SHUTDOWN);
                Intent battChanged = new Intent(ACTION_LAMP_PHONE_SHUTDOWN);
                context.sendBroadcast(battChanged);
            }
            ...
            if (intent.getAction().equals(Intent.ACTION_REBOOT)) {
                ...
                if (Lamp.DEBUG) Log.d(TAG, ACTION_LAMP_PHONE_REBOOT);
                Intent battChanged = new Intent(ACTION_LAMP_PHONE_REBOOT);
                context.sendBroadcast(battChanged);
            }
        }
    }
    ...
    private static Battery.LAMPSensorObserver awareSensor;

    public static void setSensorObserver(Battery.LAMPSensorObserver observer) {
        awareSensor = observer;
    }

    public static Battery.LAMPSensorObserver getSensorObserver() {
        return awareSensor;
    }
    ...
    public void onCreate() {
        ...
        if (Lamp.DEBUG) Log.d(TAG, "Battery service created!");
    }
    ...
    public void onDestroy() {
        ...
        ContentResolver.setSyncAutomatically(Lamp.getLAMPAccount(this), Battery_Provider.getAuthority(this), false);
        ...
        if (Lamp.DEBUG) Log.d(TAG, "Battery service terminated...");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (PERMISSIONS_OK) {
            ...
            Lamp.setSetting(this, Lamp_Preferences.STATUS_BATTERY, true);

            if (Lamp.DEBUG) Log.d(TAG, "Battery service active...");

            if (Lamp.isStudy(this)) {
                ContentResolver.setIsSyncable(Lamp.getLAMPAccount(this), Battery_Provider.getAuthority(this), 1);
                ContentResolver.setSyncAutomatically(Lamp.getLAMPAccount(this), Battery_Provider.getAuthority(this), true);
                long frequency = Long.parseLong(Lamp.getSetting(this, Lamp_Preferences.FREQUENCY_WEBSERVICE)) * 60;
                SyncRequest request = new SyncRequest.Builder()
                        .syncPeriodic(frequency, frequency / 3)
                        .setSyncAdapter(Lamp.getLAMPAccount(this), Battery_Provider.getAuthority(this))
