...
public class Lamp extends Service {
    ...
    /**
     * Used on the scheduler class to define global schedules for LAMP, SYNC and SPACE MAINTENANCE actions
     */
    ...
    public void onCreate() {
        ...
        awareActions.addAction(Lamp.ACTION_QUIT_STUDY);
        ...
        aware_account = getLAMPAccount(this);
    }

    /**
     * Return LAMP's account
     *
     * @param context
     * @return
     */
    public static Account getLAMPAccount(Context context) {
        AccountManager accountManager = (AccountManager) context.getSystemService(ACCOUNT_SERVICE);
        Account[] accounts = accountManager.getAccountsByType(Lamp_Accounts.Aware_Account.LAMP_ACCOUNT_TYPE);
        if (accounts.length > 0) {
            aware_account = accounts[0];
            return aware_account;
        }
        if (aware_account == null) {
            aware_account = new Account(Lamp_Accounts.Aware_Account.LAMP_ACCOUNT, Lamp_Accounts.Aware_Account.LAMP_ACCOUNT_TYPE);
            try {
    ...
    public void foreground(boolean enable) {
        if (enable) {
            Intent aware = new Intent(this, Lamp.class);
            PendingIntent onTap = PendingIntent.getService(this, 0, aware, 0);

            NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(this, Lamp.LAMP_NOTIFICATION_CHANNEL_SILENT);
            ...
            mBuilder = Lamp.setNotificationProperties(mBuilder, LAMP_NOTIFICATION_IMPORTANCE_SILENT);

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                mBuilder.setChannelId(Lamp.LAMP_NOTIFICATION_CHANNEL_SILENT);

            startForeground(Lamp.LAMP_FOREGROUND_SERVICE, mBuilder.build());
        } else {
            stopForeground(true);
        }
    }
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(Intent.ACTION_TIME_TICK)) { //Executed every 1-minute. OS will send this tickle automatically
                ...
                if (ts > last_time + interval_ms - 30000) {
                    ...
                    scheduler.setAction(Scheduler.ACTION_LAMP_SCHEDULER_CHECK);
    ...
    private class AsyncPing extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            //Ping LAMP's server with awareContext device's information for framework's statistics log
            Hashtable<String, String> device_ping = new Hashtable<>();
            device_ping.put(Lamp_Preferences.DEVICE_ID, Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEVICE_ID));
    ...
    public static boolean isBatteryOptimizationIgnored(Context context, String package_name) {
        ...
        if (!is_ignored) {
            NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(context, Lamp.LAMP_NOTIFICATION_CHANNEL_GENERAL);
            ...
            mBuilder = setNotificationProperties(mBuilder, LAMP_NOTIFICATION_IMPORTANCE_GENERAL);
            ...
            notManager.notify(Lamp.LAMP_BATTERY_OPTIMIZATION_ID, mBuilder.build());
        }

        Log.d(Lamp.TAG, "Battery Optimizations: " + is_ignored);
    ...
    private class AsyncStudyCheck extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            studyCheck.put(Lamp_Preferences.DEVICE_ID, Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEVICE_ID));
            studyCheck.put("study_check", "1");

            try {
                String webserver = Lamp.getSetting(getApplicationContext(), Lamp_Preferences.WEBSERVICE_SERVER);
                ...
                if (protocol.equalsIgnoreCase("https")) {
                    study_status = new Https(SSLManager.getHTTPS(getApplicationContext(), Lamp.getSetting(getApplicationContext(), Lamp_Preferences.WEBSERVICE_SERVER))).dataPOST(Lamp.getSetting(getApplicationContext(), Lamp_Preferences.WEBSERVICE_SERVER), studyCheck, true);
                } else {
                    study_status = new Http().dataPOST(Lamp.getSetting(getApplicationContext(), Lamp_Preferences.WEBSERVICE_SERVER), studyCheck, true);
                }
                ...
                try {
                    ...
                    if (!study.getString("config").equalsIgnoreCase("[]")) {
                        ...
                        Lamp.tweakSettings(getApplicationContext(), new JSONArray().put(configJSON));
                    }
        ...
        protected void onPostExecute(Boolean studyStatus) {
            ...
            if (!studyStatus) {
                sendBroadcast(new Intent(Lamp.ACTION_QUIT_STUDY));
            }
        }
    }
    ...
    private void get_device_info() {
        ...
        if (awareContextDevice == null || !awareContextDevice.moveToFirst()) {
            ...
            rowData.put(Aware_Device.DEVICE_ID, Lamp.getSetting(this, Lamp_Preferences.DEVICE_ID));
            ...
            rowData.put(Aware_Device.LABEL, Lamp.getSetting(this, Lamp_Preferences.DEVICE_LABEL));

            try {
                getContentResolver().insert(Aware_Device.CONTENT_URI, rowData);

                Intent deviceData = new Intent(ACTION_LAMP_DEVICE_INFORMATION);
                sendBroadcast(deviceData);

                if (Lamp.DEBUG) Log.d(TAG, "Device information:" + rowData.toString());
    ...
    public static boolean isStudy(Context c) {
        ...
        Cursor study = c.getContentResolver().query(Lamp_Provider.Aware_Studies.CONTENT_URI, null, null, null,
                Lamp_Provider.Aware_Studies.STUDY_TIMESTAMP + " DESC LIMIT 1");

        if (study != null && study.moveToFirst()) {
            participant = (study.getDouble(study.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_EXIT)) == 0
                    && study.getDouble(study.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_JOINED)) > 0); //joined and still enrolled
        }
    ...
    public static void debug(Context c, String message) {
        ...
        if (!Lamp.isStudy(c)) return;

        ContentValues log = new ContentValues();
        log.put(Lamp_Provider.Aware_Log.LOG_TIMESTAMP, System.currentTimeMillis());
        log.put(Lamp_Provider.Aware_Log.LOG_DEVICE_ID, Lamp.getSetting(c, Lamp_Preferences.DEVICE_ID));
        log.put(Lamp_Provider.Aware_Log.LOG_MESSAGE, message);

        if (Lamp.DEBUG) Log.d(TAG, "Aware_Log: \n" + log.toString());

        c.getContentResolver().insert(Lamp_Provider.Aware_Log.CONTENT_URI, log);
    }
    ...
    public static Cursor getStudy(Context c, String study_url) {
        return c.getContentResolver().query(Lamp_Provider.Aware_Studies.CONTENT_URI, null, Lamp_Provider.Aware_Studies.STUDY_URL + " LIKE '" + study_url + "%' AND " + Lamp_Provider.Aware_Studies.STUDY_EXIT + "=0", null, Lamp_Provider.Aware_Studies.STUDY_TIMESTAMP + " DESC LIMIT 1");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            ...
            if (Lamp.DEBUG) Log.d(TAG, "LAMP framework is active...");
            ...
            if (prefs.getAll().isEmpty() && Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEVICE_ID).length() == 0) {
            ...
            for (Map.Entry<String, ?> entry : defaults.entrySet()) {
                if (Lamp.getSetting(getApplicationContext(), entry.getKey(), "com.aware.phone").length() == 0) {
                    Lamp.setSetting(getApplicationContext(), entry.getKey(), entry.getValue(), "com.aware.phone"); //default LAMP settings
                }
            }

            if (Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEVICE_ID).length() == 0) {
                UUID uuid = UUID.randomUUID();
                Lamp.setSetting(getApplicationContext(), Lamp_Preferences.DEVICE_ID, uuid.toString(), "com.aware.phone");
            }

            if (Lamp.getSetting(getApplicationContext(), Lamp_Preferences.WEBSERVICE_SERVER).length() == 0) {
                Lamp.setSetting(getApplicationContext(), Lamp_Preferences.WEBSERVICE_SERVER, "https://api.awareframework.com/index.php");
            }

            DEBUG = Lamp.getSetting(this, Lamp_Preferences.DEBUG_FLAG).equals("true");
            TAG = Lamp.getSetting(this, Lamp_Preferences.DEBUG_TAG).length() > 0 ? Lamp.getSetting(this, Lamp_Preferences.DEBUG_TAG) : TAG;

            get_device_info();

            if (Lamp.getSetting(getApplicationContext(), Lamp_Preferences.LAMP_DONATE_USAGE).equals("true")) {
            ...
            if (intent != null && intent.getAction() != null) {
                if (intent.getAction().equalsIgnoreCase(ACTION_LAMP_STUDY_COMPLIANCE)) {
                    ...
                    if (studyCheck == null && Lamp.isStudy(getApplicationContext())) {
                        studyCheck = new AsyncStudyCheck();
                        studyCheck.execute();
                    }
                }

                if (intent.getAction().equalsIgnoreCase(ACTION_LAMP_KEEP_ALIVE)) {
                    startLAMP(getApplicationContext());
            ...
            if (Lamp.isStudy(this)) {
                ContentResolver.setIsSyncable(Lamp.getLAMPAccount(this), Lamp_Provider.getAuthority(this), 1);
                ContentResolver.setSyncAutomatically(Lamp.getLAMPAccount(this), Lamp_Provider.getAuthority(this), true);
                long frequency = Long.parseLong(Lamp.getSetting(this, Lamp_Preferences.FREQUENCY_WEBSERVICE)) * 60;
                SyncRequest request = new SyncRequest.Builder()
                        .syncPeriodic(frequency, frequency / 3)
                        .setSyncAdapter(Lamp.getLAMPAccount(this), Lamp_Provider.getAuthority(this))
                        .setExtras(new Bundle()).build();
                ContentResolver.requestSync(request);
            }

        } else { //storage is not available, stop plugins and sensors
            stopLAMP(getApplicationContext());
    ...
    public static void checkBatteryLeft(Context context, boolean dismiss) {
        ...
        if (Lamp.getSetting(context, Lamp_Preferences.REMIND_TO_CHARGE).equals("true")) {
            ...
            if (dismiss) {
                ...
                if (batt != null && batt.getExtras() != null) {
                    ...
                    if (extras.getInt(BatteryManager.EXTRA_LEVEL) <= 15 && extras.getInt(BatteryManager.EXTRA_STATUS) != BatteryManager.BATTERY_STATUS_CHARGING) {
                        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(context.getApplicationContext(), Lamp.LAMP_NOTIFICATION_CHANNEL_GENERAL);
                        mBuilder.setSmallIcon(R.drawable.ic_stat_aware_recharge);
                        ...
                        mBuilder.setOnlyAlertOnce(true); //notify the user only once
                        mBuilder.setDefaults(NotificationCompat.DEFAULT_ALL);
                        mBuilder = Lamp.setNotificationProperties(mBuilder, LAMP_NOTIFICATION_IMPORTANCE_GENERAL);
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                            mBuilder.setChannelId(Lamp.LAMP_NOTIFICATION_CHANNEL_GENERAL);
    ...
    public synchronized static void stopPlugin(final Context context, final String package_name) {
        ...
        if (packageInfo != null) {
            ...
            if (context.getPackageName().equals("com.aware.phone") || context.getResources().getBoolean(R.bool.standalone)) {
                context.sendBroadcast(new Intent(Lamp.ACTION_LAMP_UPDATE_PLUGINS_INFO)); //sync the Plugins Manager UI for running statuses
            }

            ComponentName componentName;
            if (packageInfo.versionName.equals("bundled")) {
                componentName = new ComponentName(context.getPackageName(), package_name + ".Plugin");
                if (Lamp.DEBUG) Log.d(Lamp.TAG, "Stopping bundled: " + componentName.toString());
            } else {
                componentName = new ComponentName(package_name, package_name + ".Plugin");
                if (Lamp.DEBUG) Log.d(Lamp.TAG, "Stopping external: " + componentName.toString());
            }
    ...
    public synchronized static void startPlugin(final Context context, final String package_name) {
        ...
        if (packageInfo != null) {
            ...
            if (packageInfo.versionName.equals("bundled")) {
                componentName = new ComponentName(context.getPackageName(), package_name + ".Plugin");
                if (Lamp.DEBUG)
                    Log.d(Lamp.TAG, "Initializing bundled: " + componentName.toString());
            } else {
                componentName = new ComponentName(package_name, package_name + ".Plugin");
                if (Lamp.DEBUG)
                    Log.d(Lamp.TAG, "Initializing external: " + componentName.toString());
            }
    ...
    /**
     * Requests the download of a plugin given the package name from LAMP webservices or the Play Store otherwise
     *
     * @param context
     * @param package_name
     * @param is_update
    ...
    public static String getSetting(Context context, String key) {
        ...
        global_settings.add(Lamp_Preferences.DEBUG_FLAG);
        global_settings.add(Lamp_Preferences.DEBUG_TAG);
        global_settings.add(Lamp_Preferences.DEVICE_ID);
        global_settings.add(Lamp_Preferences.DEVICE_LABEL);
        global_settings.add(Lamp_Preferences.STATUS_WEBSERVICE);
        global_settings.add(Lamp_Preferences.FREQUENCY_WEBSERVICE);
        global_settings.add(Lamp_Preferences.WEBSERVICE_WIFI_ONLY);
        global_settings.add(Lamp_Preferences.WEBSERVICE_SERVER);
        global_settings.add(Lamp_Preferences.WEBSERVICE_SIMPLE);
        ...
        if (context.getResources().getBoolean(R.bool.standalone)) {
            ...
            global_settings.add(Lamp_Preferences.MQTT_USERNAME);
            global_settings.add(Lamp_Preferences.MQTT_PASSWORD);
            global_settings.add(Lamp_Preferences.MQTT_SERVER);
            global_settings.add(Lamp_Preferences.MQTT_PORT);
            global_settings.add(Lamp_Preferences.MQTT_KEEP_ALIVE);
            global_settings.add(Lamp_Preferences.MQTT_QOS);
        }
    ...
    public static void setSetting(Context context, String key, Object value) {
        ...
        if (key.equals(Lamp_Preferences.DEVICE_ID) && Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID).length() > 0) {
            Log.d(Lamp.TAG, "LAMP UUID: " + Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID) + " in " + context.getPackageName());
            return;
        }

        if (key.equals(Lamp_Preferences.DEVICE_LABEL) && ((String) value).length() > 0) {
            ContentValues newLabel = new ContentValues();
            newLabel.put(Lamp_Provider.Aware_Device.LABEL, (String) value);
            context.getApplicationContext().getContentResolver().update(Lamp_Provider.Aware_Device.CONTENT_URI, newLabel, Lamp_Provider.Aware_Device.DEVICE_ID + " LIKE '" + Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID) + "'", null);
        }
        ...
        if (qry != null && qry.moveToFirst()) {
            try {
                if (!qry.getString(qry.getColumnIndex(Aware_Settings.SETTING_VALUE)).equals(value.toString())) {
                    ...
                    if (Lamp.DEBUG) Log.d(Lamp.TAG, "Updated: " + key + "=" + value);
                }
            } catch (SQLiteException e) {
                if (Lamp.DEBUG) Log.d(TAG, e.getMessage());
            } catch (SQLException e) {
            ...
            try {
                ...
                if (Lamp.DEBUG) Log.d(Lamp.TAG, "Added: " + key + "=" + value);
            } catch (SQLiteException e) {
                if (Lamp.DEBUG) Log.d(TAG, e.getMessage());
            } catch (SQLException e) {
                if (Lamp.DEBUG) Log.d(TAG, e.getMessage());
    ...
    public static void setSetting(Context context, String key, Object value, String package_name) {
        ...
        if (key.equals(Lamp_Preferences.DEVICE_ID) && Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID).length() > 0) {
            Log.d(Lamp.TAG, "LAMP UUID: " + Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID) + " in " + package_name);
            return;
        }
        ...
        if (key.equals(Lamp_Preferences.DEVICE_LABEL) && ((String) value).length() > 0) {
            ContentValues newLabel = new ContentValues();
            newLabel.put(Lamp_Provider.Aware_Device.LABEL, (String) value);
            context.getContentResolver().update(Lamp_Provider.Aware_Device.CONTENT_URI, newLabel, Lamp_Provider.Aware_Device.DEVICE_ID + " LIKE '" + Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID) + "'", null);
        }
        ...
        if (qry != null && qry.moveToFirst()) {
            try {
                if (!qry.getString(qry.getColumnIndex(Aware_Settings.SETTING_VALUE)).equals(value.toString())) {
                    ...
                    if (Lamp.DEBUG)
                        Log.d(Lamp.TAG, "Updated: " + key + "=" + value + " in " + package_name);
                }
            } catch (SQLiteException e) {
                if (Lamp.DEBUG) Log.d(TAG, e.getMessage());
            } catch (SQLException e) {
            ...
            try {
                ...
                if (Lamp.DEBUG)
                    Log.d(Lamp.TAG, "Added: " + key + "=" + value + " in " + package_name);
            } catch (SQLiteException e) {
                if (Lamp.DEBUG) Log.d(TAG, e.getMessage());
            } catch (SQLException e) {
                if (Lamp.DEBUG) Log.d(TAG, e.getMessage());
    ...
    protected static void tweakSettings(Context c, JSONArray serverConfig) {
        ...
        Cursor study = getStudy(c, Lamp.getSetting(c, Lamp_Preferences.WEBSERVICE_SERVER));
        int study_id = 0;
        if (study != null && study.moveToFirst()) {
            try {
                localConfig = new JSONArray(study.getString(study.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_CONFIG)));
                study_id = study.getInt(study.getColumnIndex(Lamp_Provider.Aware_Studies._ID));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
                ...
                if (sensorSync.get(0).length() > 0 || sensorSync.get(1).length() > 0) {
                    ...
                    for (int i = 0; i < enabled.length(); i++) {
                        try {
                            ...
                            if (sensor_config.getString("setting").contains("status")) {
                                Lamp.setSetting(c, sensor_config.getString("setting"), true, "com.aware.phone");
                            } else
                                Lamp.setSetting(c, sensor_config.getString("setting"), sensor_config.getString("value"), "com.aware.phone");
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
                    }
                    ...
                    for (int i = 0; i < disabled.length(); i++) {
                        try {
                            ...
                            if (sensor_config.getString("setting").contains("status")) {
                                Lamp.setSetting(c, sensor_config.getString("setting"), false, "com.aware.phone");
                            } else
                                Lamp.setSetting(c, sensor_config.getString("setting"), sensor_config.getString("value"), "com.aware.phone");
                        } catch (JSONException e) {
                            e.printStackTrace();
                        ...
                        try {
                            ...
                            for (int j = 0; j < plugin_settings.length(); j++) {
                                ...
                                if (plugin_set.getString("setting").contains("status")) {
                                    Lamp.setSetting(c, plugin_set.getString("setting"), true, package_name);
                                } else
                                    Lamp.setSetting(c, plugin_set.getString("setting"), plugin_set.getString("value"), package_name);
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
                    ...
                    for (int i = 0; i < disabled.length(); i++) {
                        try {
                            ...
                            for (int j = 0; j < plugin_settings.length(); j++) {
                                JSONObject plugin_set = plugin_settings.getJSONObject(j);
                                if (plugin_set.getString("setting").contains("status")) {
                                    Lamp.setSetting(c, plugin_set.getString("setting"), false, package_name);
                                } else
                                    Lamp.setSetting(c, plugin_set.getString("setting"), plugin_set.getString("value"), package_name);
                            }
                        } catch (JSONException e) {
                    ...
                    if (config_changed) {
                        ...
                        for (int i = 0; i < enabled.length(); i++) {
                            ...
                            if (PluginsManager.isInstalled(c, enabled.getJSONObject(i).getString("plugin")) != null) {
                                Lamp.startPlugin(c, enabled.getJSONObject(i).getString("plugin"));
                            } else
                                Lamp.downloadPlugin(c, enabled.getJSONObject(i).getString("plugin"), null, false);
                        }
                        ...
                        for (int i = 0; i < disabled.length(); i++) {
                            ...
                            for (int j = 0; j < localConfig.getJSONObject(0).getJSONArray("plugins").length(); j++) {
                                ...
                                if (removed.getString("plugin").equalsIgnoreCase(local.getString("plugin"))) {
                                    ...
                                    Lamp.stopPlugin(c, removed.getString("plugin"));
                                }
                            }
                        }
                    }
        ...
        if (config_changed) {
            ...
            newCfg.put(Lamp_Provider.Aware_Studies.STUDY_CONFIG, localConfig.toString());
            c.getContentResolver().update(Lamp_Provider.Aware_Studies.CONTENT_URI, newCfg, Lamp_Provider.Aware_Studies._ID + "=" + study_id, null);

            Intent aware = new Intent(c, Lamp.class);
    ...
    private static ArrayList<JSONArray> sensorDiff(Context context, JSONArray server, JSONArray local) throws JSONException {
        ...
        optionalSensors.put(Lamp_Preferences.STATUS_ACCELEROMETER, Sensor.TYPE_ACCELEROMETER);
        optionalSensors.put(Lamp_Preferences.STATUS_SIGNIFICANT_MOTION, Sensor.TYPE_ACCELEROMETER);
        optionalSensors.put(Lamp_Preferences.STATUS_BAROMETER, Sensor.TYPE_PRESSURE);
        optionalSensors.put(Lamp_Preferences.STATUS_GRAVITY, Sensor.TYPE_GRAVITY);
        optionalSensors.put(Lamp_Preferences.STATUS_GYROSCOPE, Sensor.TYPE_GYROSCOPE);
        optionalSensors.put(Lamp_Preferences.STATUS_LIGHT, Sensor.TYPE_LIGHT);
        optionalSensors.put(Lamp_Preferences.STATUS_LINEAR_ACCELEROMETER, Sensor.TYPE_LINEAR_ACCELERATION);
        optionalSensors.put(Lamp_Preferences.STATUS_MAGNETOMETER, Sensor.TYPE_MAGNETIC_FIELD);
        optionalSensors.put(Lamp_Preferences.STATUS_PROXIMITY, Sensor.TYPE_PROXIMITY);
        optionalSensors.put(Lamp_Preferences.STATUS_ROTATION, Sensor.TYPE_ROTATION_VECTOR);
        optionalSensors.put(Lamp_Preferences.STATUS_TEMPERATURE, Sensor.TYPE_AMBIENT_TEMPERATURE);
                ...
                if (request != null) {
                    ...
                    try {
                        ...
                        if (dbStudy == null || !dbStudy.moveToFirst()) {
                            ...
                            studyData.put(Lamp_Provider.Aware_Studies.STUDY_TIMESTAMP, System.currentTimeMillis());
                            studyData.put(Lamp_Provider.Aware_Studies.STUDY_JOINED, System.currentTimeMillis());
                            studyData.put(Lamp_Provider.Aware_Studies.STUDY_EXIT, 0);
                            studyData.put(Lamp_Provider.Aware_Studies.STUDY_KEY, study_id);
                            studyData.put(Lamp_Provider.Aware_Studies.STUDY_API, study_api_key);
                            studyData.put(Lamp_Provider.Aware_Studies.STUDY_URL, full_url);
                            studyData.put(Lamp_Provider.Aware_Studies.STUDY_PI, studyInfo.getString("researcher_first") + " " + studyInfo.getString("researcher_last") + "\nContact: " + studyInfo.getString("researcher_contact"));
                        ...
                        for (int i = 0; i < sensors.length(); i++) {
                            try {
                                ...
                                Lamp.setSetting(getApplicationContext(), sensor_config.getString("setting"), sensor_config.get("value"), package_name);
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                        }
                        ...
                        for (int i = 0; i < plugins.length(); i++) {
                            try {
                                ...
                                for (int j = 0; j < plugin_settings.length(); j++) {
                                    ...
                                    if (getApplicationContext().getResources().getBoolean(R.bool.standalone))
                                        package_name = getApplicationContext().getPackageName();
                                    Lamp.setSetting(getApplicationContext(), plugin_setting.getString("setting"), plugin_setting.get("value"), package_name);
                        ...
                        for (String package_name : enabled_plugins) {
                            ...
                            if (installed != null) {
                                Lamp.startPlugin(getApplicationContext(), package_name);
                            } else {
                                Lamp.downloadPlugin(getApplicationContext(), package_name, null, false);
                            }
                        }
                        ...
                        sendBroadcast(new Intent(Lamp.ACTION_JOINED_STUDY));
    ...
    public static void reset(Context context) {
        String device_id = Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID);
        String device_label = Lamp.getSetting(context, Lamp_Preferences.DEVICE_LABEL);
        ...
        Map<String, ?> defaults = prefs.getAll();
        for (Map.Entry<String, ?> entry : defaults.entrySet()) {
            Lamp.setSetting(context, entry.getKey(), entry.getValue(), "com.aware.phone");
        }

        //Keep previous LAMP Device ID and label
        Lamp.setSetting(context, Lamp_Preferences.DEVICE_ID, device_id, "com.aware.phone");
        Lamp.setSetting(context, Lamp_Preferences.DEVICE_LABEL, device_label, "com.aware.phone");
        ...
        Cursor enabled_plugins = context.getContentResolver().query(Aware_Plugins.CONTENT_URI, null, Aware_Plugins.PLUGIN_STATUS + "=" + Lamp_Plugin.STATUS_PLUGIN_ON, null, null);
        ...
        if (active_plugins.size() > 0) {
            ...
            if (Lamp.DEBUG) Log.w(TAG, "LAMP plugins disabled...");
        }
    ...
    public static class AndroidPackageMonitor extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(Intent.ACTION_PACKAGE_ADDED)) {
                ...
                Intent installed = new Intent(ACTION_LAMP_PLUGIN_INSTALLED);
                installed.putExtra(EXTRA_PLUGIN, packageName);
                context.sendBroadcast(installed);

                //Updating a package
                if (extras.getBoolean(Intent.EXTRA_REPLACING)) {
                    if (Lamp.DEBUG) Log.d(TAG, packageName + " is updating!");

                    //Check study compliance
                    if (Lamp.isStudy(context)) {
                        Cursor studyInfo = Lamp.getStudy(context, Lamp.getSetting(context, Lamp_Preferences.WEBSERVICE_SERVER));
                        if (studyInfo != null && studyInfo.moveToFirst()) {
                            try {
                                JSONArray studyConfig = new JSONArray(studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_CONFIG)));
                                JSONArray plugins = new JSONArray();
                                ...
                                for (int j = 0; j < plugins.length(); j++) {
                                    ...
                                    if (package_name.equalsIgnoreCase(packageName)) {
                                        ...
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_DEVICE_ID, Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_TIMESTAMP, System.currentTimeMillis());
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_KEY, studyInfo.getInt(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_KEY)));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_API, studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_API)));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_URL, studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_URL)));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_PI, studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_PI)));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_CONFIG, studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_CONFIG)));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_JOINED, studyInfo.getLong(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_JOINED)));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_TITLE, studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_TITLE)));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_DESCRIPTION, studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_DESCRIPTION)));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_COMPLIANCE, "updated plugin: " + package_name);

                                        context.getContentResolver().insert(Lamp_Provider.Aware_Studies.CONTENT_URI, complianceEntry);

                                        if (Lamp.DEBUG)
                                            Log.d(Lamp.TAG, "Study compliance check: " + complianceEntry.toString());
                                    }
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                    ...
                    if (current_status != null && current_status.moveToFirst()) {
                        if (current_status.getInt(current_status.getColumnIndex(Aware_Plugins.PLUGIN_STATUS)) == PluginsManager.PLUGIN_UPDATED) { //was updated, set to active now
                            rowData.put(Aware_Plugins.PLUGIN_STATUS, Lamp_Plugin.STATUS_PLUGIN_ON);
                        }
                    }
                    ...
                    //Start plugin
                    Lamp.startPlugin(context, packageName);
                    return;
                }
                ...
                try {
                    ...
                    if (Lamp.DEBUG)
                        Log.d(TAG, "LAMP plugin added and activated:" + app.packageName);
                            ...
                            try {
                                ...
                                for (int j = 0; j < plugins.length(); j++) {
                                    ...
                                    if (package_name.equalsIgnoreCase(packageName)) {
                                        ...
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_TITLE, studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_TITLE)));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_DESCRIPTION, studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_DESCRIPTION)));
                                        complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_COMPLIANCE, "installed plugin: " + package_name);
                    ...
                    Lamp.startPlugin(context, app.packageName);
            ...
            if (intent.getAction().equals(Intent.ACTION_PACKAGE_REMOVED)) {
                ...
                Intent installed = new Intent(ACTION_LAMP_PLUGIN_UNINSTALLED);
                installed.putExtra(EXTRA_PLUGIN, packageName);
                context.sendBroadcast(installed);
                        ...
                        try {
                            ...
                            for (int j = 0; j < plugins.length(); j++) {
                                ...
                                if (package_name.equalsIgnoreCase(packageName)) {
                                    ...
                                    complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_TITLE, studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_TITLE)));
                                    complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_DESCRIPTION, studyInfo.getString(studyInfo.getColumnIndex(Lamp_Provider.Aware_Studies.STUDY_DESCRIPTION)));
                                    complianceEntry.put(Lamp_Provider.Aware_Studies.STUDY_COMPLIANCE, "uninstalled plugin: " + package_name);
                ...
                if (Lamp.DEBUG) Log.d(TAG, "LAMP plugin removed:" + packageName);
            }
        }
    }

    /**
     * BroadcastReceiver that monitors for LAMP framework actions:
     * Aware#ACTION_LAMP_ACTION_QUIT_STUDY: quits a study
     * Aware#ACTION_LAMP_SYNC_DATA: send the data remotely
     *
     * @author denzil
     */
    ...
    public static class Aware_Broadcaster extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(Lamp.ACTION_QUIT_STUDY)) {
                Lamp.reset(context);
            }
            if (intent.getAction().equals(Lamp.ACTION_LAMP_SYNC_DATA)) {
                Bundle sync = new Bundle();
                sync.putBoolean(ContentResolver.SYNC_EXTRAS_MANUAL, true);
                sync.putBoolean(ContentResolver.SYNC_EXTRAS_EXPEDITED, true);
                ContentResolver.requestSync(Lamp.getLAMPAccount(context), Lamp_Provider.getAuthority(context), sync);
            }
        }
    }

    /**
     * Checks if we have access to the storage of the device. Turns off LAMP when we don't, turns it back on when available again.
     */
    ...
    public static class Storage_Broadcaster extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(Intent.ACTION_MEDIA_MOUNTED)) {
                if (Lamp.DEBUG) Log.d(TAG, "Resuming LAMP data logging...");
                Intent aware = new Intent(context, Lamp.class);
                context.startService(aware);
            }
            if (intent.getAction().equals(Intent.ACTION_MEDIA_UNMOUNTED)) {
                if (Lamp.DEBUG)
                    Log.w(TAG, "Stopping LAMP data logging until the SDCard is available again...");
                Lamp.stopLAMP(context);
            }
        }
    }
...
//    }
    ...
    private static void complianceStatus(Context context) {
        ...
        try {
            ...
            Lamp.debug(context, complianceStatus.toString());
     ...
     */
    public static void startLAMP(Context context) {

        startScheduler(context);

        if (Lamp.getSetting(context, Lamp_Preferences.STATUS_SIGNIFICANT_MOTION).equals("true")) {
        ...
        if (Lamp.getSetting(context, Lamp_Preferences.STATUS_LIGHT).equals("true")) {
            startLight(context);
        } else stopLight(context);

        if (Lamp.getSetting(context, Lamp_Preferences.STATUS_PROXIMITY).equals("true")) {
            startProximity(context);
        } else stopProximity(context);

        if (Lamp.getSetting(context, Lamp_Preferences.STATUS_MAGNETOMETER).equals("true")) {
            startMagnetometer(context);
        } else stopMagnetometer(context);

        if (Lamp.getSetting(context, Lamp_Preferences.STATUS_BAROMETER).equals("true")) {
