...
public class Scheduler extends Lamp_Sensor {

    public static final String ACTION_LAMP_SCHEDULER_CHECK = "ACTION_LAMP_SCHEDULER_CHECK";
    public static final String ACTION_LAMP_SCHEDULER_TRIGGERED = "ACTION_LAMP_SCHEDULER_TRIGGERED";
    ...
    private static String TAG = "LAMP::Scheduler";
    ...
    public static void saveSchedule(Context context, Schedule schedule) {
        try {
            ...
            if (schedule.getRandom().length() != 0 && schedule.getTimer() == -1) {
                ...
                random_seed += "-" + Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID);
                ...
                data.put(Scheduler_Provider.Scheduler_Data.DEVICE_ID, Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID));
    ...
    public static void setSchedules(Context c, JSONArray schedules) {
        for (int i = 0; i < schedules.length(); i++) {
            try {
                ...
                e.printStackTrace();
                if (Lamp.DEBUG) Log.d(Scheduler.TAG, "Error in JSON: " + e.getMessage());
            }
        }

        //Apply new schedules immediately
        Lamp.startScheduler(c);
    }
    ...
    private void performAction(Schedule schedule) {
        try {
            ...
            Intent scheduler_action = new Intent(Scheduler.ACTION_LAMP_SCHEDULER_TRIGGERED);
            scheduler_action.putExtra(EXTRA_SCHEDULER_ID, schedule.getScheduleID());
            sendBroadcast(scheduler_action);

            Lamp.debug(this, "Scheduler triggered: " + schedule.getScheduleID() + " schedule: " + schedule.build().toString() + " package: " + getPackageName());
    ...
    public static class Schedule {
         ...
         *
         * @param broadcast e.g., ACTION_LAMP_CALL_ACCEPTED runs this schedule when the user has answered a phone call
         */
    ...
    private class DBObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            ...
            if (DEBUG)
                Log.d(Lamp.TAG, "Checking condition : " + data.toString() + " where: " + condition);
            ...
            if (this.data != null && this.condition.length() > 0) {
                ...
                if (condition_met) {
                    if (is_trigger(schedule)) {
                        performAction(schedule);
                        if (DEBUG)
                            Log.d(Lamp.TAG, "Condition triggered: " + data.toString() + " where: " + condition);
                    }
                }
            }
        }
