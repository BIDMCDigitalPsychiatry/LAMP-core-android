...
public class PluginsManager {
    ...
    public static PackageInfo isInstalled(Context context, String package_name) {
        ...
        try {
            ...
            Cursor cached = context.getContentResolver().query(Lamp_Provider.Aware_Plugins.CONTENT_URI, null, Lamp_Provider.Aware_Plugins.PLUGIN_PACKAGE_NAME + " LIKE '" + package_name + "'", null, null);
            if (cached == null || !cached.moveToFirst()) {
                ...
                rowData.put(Lamp_Provider.Aware_Plugins.PLUGIN_AUTHOR, "Bundle");
                rowData.put(Lamp_Provider.Aware_Plugins.PLUGIN_DESCRIPTION, "Bundled with " + context.getPackageName());
                rowData.put(Lamp_Provider.Aware_Plugins.PLUGIN_NAME, package_name);
                rowData.put(Lamp_Provider.Aware_Plugins.PLUGIN_PACKAGE_NAME, package_name);
                rowData.put(Lamp_Provider.Aware_Plugins.PLUGIN_STATUS, Lamp_Plugin.STATUS_PLUGIN_OFF);
                rowData.put(Lamp_Provider.Aware_Plugins.PLUGIN_VERSION, 1);
                context.getContentResolver().insert(Lamp_Provider.Aware_Plugins.CONTENT_URI, rowData);
                if (Lamp.DEBUG) Log.d(Lamp.TAG, "Added bundled plugin: " + package_name + " to " + context.getPackageName());
            }
    ...
    public static boolean isEnabled(Context context, String pkg_name) {
        ...
        if (enabled != null && enabled.moveToFirst()) {
            result = (enabled.getInt(enabled.getColumnIndex(Aware_Plugins.PLUGIN_STATUS)) == Lamp_Plugin.STATUS_PLUGIN_ON);
        }
    ...
    public static boolean isDisabled(Context context, String pkg_name) {
        ...
        if (disabled != null && disabled.moveToFirst()) {
            result = (disabled.getInt(disabled.getColumnIndex(Aware_Plugins.PLUGIN_STATUS)) == Lamp_Plugin.STATUS_PLUGIN_OFF);
        }
    ...
    /**
     * Tell LAMP that this plugin should be enabled
     * @param context
     * @param pkg_name
     */
    ...
    /**
     * Tell LAMP that this plugin should be disabled
     * @param context
     * @param pkg_name
     */
    public static void disablePlugin(Context context, String pkg_name) {
        ContentValues rowData = new ContentValues();
        rowData.put(Aware_Plugins.PLUGIN_STATUS, Lamp_Plugin.STATUS_PLUGIN_OFF);
        context.getContentResolver().update(Aware_Plugins.CONTENT_URI, rowData, Aware_Plugins.PLUGIN_PACKAGE_NAME + " LIKE '" + pkg_name + "'", null);
    }

    /**
     * Check if plugin is on LAMP's database already
     *
     * @param context
     * @param pkg_name
     * @return
    ...
    public static String getPluginVersion(Context context, String package_name) {
        try {
            ...
            if (Lamp.DEBUG) Log.d(Lamp.TAG, e.getMessage());
        }
        return "";
    }
