...
public class DownloadPluginService extends IntentService {
    ...
    public DownloadPluginService() { super(Lamp.TAG + " Plugin Downloader"); }
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        String study_url = Lamp.getSetting(getApplicationContext(), Lamp_Preferences.WEBSERVICE_SERVER);
        if (intent.hasExtra("study_url"))
            study_url = intent.getStringExtra("study_url");


        if( Lamp.DEBUG ) Log.d(Lamp.TAG, "Trying to download: " + package_name +" using server: " +study_url);
        ...
        if( response != null ) {
            try {
                ...
                File folders = new File(Environment.getExternalStoragePublicDirectory("LAMP/plugins").toString());
                folders.mkdirs();

                String package_url = study_host + json_package.getString("package_path") + json_package.getString("package_name");

                NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(getApplicationContext(), Lamp.LAMP_NOTIFICATION_CHANNEL_DATASYNC);
                mBuilder.setSmallIcon(R.drawable.ic_action_aware_plugins);
                mBuilder.setContentTitle("LAMP Plugin");
                ...
                Ion.with(getApplicationContext()).load(package_url).noCache()
                        .write(new File(Environment.getExternalStoragePublicDirectory("LAMP/plugins/" + json_package.getString("package_name")).toString()))
