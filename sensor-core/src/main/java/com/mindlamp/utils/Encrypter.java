...
public class Encrypter {
        ...
        if (hashProgram.length() > 0) {
            ...
            for (String command: hashProgramCommands) {
                if (command.equals("salt=device_id")) {
                    ...
                    clear = clear + Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID);
    ...
    public static final String hash(Context context, String clear) {
        String hashProgram = Lamp.getSetting(context.getApplicationContext(), Lamp_Preferences.HASH_FUNCTION);
        if (hashProgram.equals("")) {
            // Default if unset
            hashProgram = "SHA-1";
        }

        String HASH_SALT = Lamp.getSetting(context.getApplicationContext(), Lamp_Preferences.HASH_SALT);
    ...
    public static final String hashPhone(Context context, String clear) {
        String hashProgram = Lamp.getSetting(context.getApplicationContext(), Lamp_Preferences.HASH_FUNCTION_PHONE);
        return _hashProgram(context, clear, hashProgram);
    }
    ...
    public static final String hashMac(Context context, String clear) {
        String hashProgram = Lamp.getSetting(context.getApplicationContext(), Lamp_Preferences.HASH_FUNCTION_MAC);
        if (hashProgram.equals("")) {
            hashProgram = "clear";
        }
        return _hashProgram(context, clear, hashProgram);
    ...
    public static final String hashSsid(Context context, String clear) {
        String hashProgram = Lamp.getSetting(context.getApplicationContext(), Lamp_Preferences.HASH_FUNCTION_SSID);
        if (hashProgram.equals("")) {
            hashProgram = "clear";
        }
        return _hashProgram(context, clear, hashProgram);
