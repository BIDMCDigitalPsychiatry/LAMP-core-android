...
import com.mindlamp.providers.Temperature_Provider;
import com.mindlamp.providers.Temperature_Provider.Temperature_Data;
import com.mindlamp.providers.Temperature_Provider.Temperature_Sensor;
...
/**
 * LAMP Temperature module
 * - Temperature raw data in ambient room temperature in degree Celsius
 * - Temperature sensor information
 *
 * @author df
 */
public class Temperature extends Lamp_Sensor implements SensorEventListener {

    /**
     * Logging tag (default = "LAMP::Temperature")
     */
    private static String TAG = "LAMP::Temperature";
     ...
     */
    public static final String ACTION_LAMP_TEMPERATURE = "ACTION_LAMP_TEMPERATURE";
    public static final String ACTION_LAMP_TEMPERATURE_LABEL = "ACTION_LAMP_TEMPERATURE_LABEL";
    ...
    public static class DataLabel extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(ACTION_LAMP_TEMPERATURE_LABEL)) {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        rowData.put(Temperature_Data.DEVICE_ID, Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEVICE_ID));
            ...
            if (!Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEBUG_DB_SLOW).equals("true")) {
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Intent accelData = new Intent(ACTION_LAMP_TEMPERATURE);
    ...
    private static Temperature.LAMPSensorObserver awareSensor;

    public static void setSensorObserver(Temperature.LAMPSensorObserver observer) {
        awareSensor = observer;
    }

    public static Temperature.LAMPSensorObserver getSensorObserver() {
        return awareSensor;
    }
    ...
    private void saveSensorDevice(Sensor sensor) {
        ...
        if (sensorInfo == null || !sensorInfo.moveToFirst()) {
            ContentValues rowData = new ContentValues();
            rowData.put(Temperature_Sensor.DEVICE_ID, Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEVICE_ID));
            ...
            if (Lamp.DEBUG) Log.d(TAG, "Temperature sensor info: " + rowData.toString());
        }
    ...
    public void onCreate() {
        ...
        filter.addAction(ACTION_LAMP_TEMPERATURE_LABEL);
        ...
        if (Lamp.DEBUG) Log.d(TAG, "Temperature service created!");
    }
    ...
    public void onDestroy() {
        ...
        ContentResolver.setSyncAutomatically(Lamp.getLAMPAccount(this), Temperature_Provider.getAuthority(this), false);
        ...
        if (Lamp.DEBUG) Log.d(TAG, "Temperature service terminated...");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (PERMISSIONS_OK) {
            if (mTemperature == null) {
                ...
                Lamp.setSetting(this, Lamp_Preferences.STATUS_TEMPERATURE, false);
                stopSelf();
            } else {
                DEBUG = Lamp.getSetting(this, Lamp_Preferences.DEBUG_FLAG).equals("true");
                Lamp.setSetting(this, Lamp_Preferences.STATUS_TEMPERATURE, true);

                saveSensorDevice(mTemperature);

                if (Lamp.getSetting(this, Lamp_Preferences.FREQUENCY_TEMPERATURE).length() == 0) {
                    Lamp.setSetting(this, Lamp_Preferences.FREQUENCY_TEMPERATURE, 200000);
                }

                if (Lamp.getSetting(this, Lamp_Preferences.THRESHOLD_TEMPERATURE).length() == 0) {
                    Lamp.setSetting(this, Lamp_Preferences.THRESHOLD_TEMPERATURE, 0.0);
                }

                int new_frequency = Integer.parseInt(Lamp.getSetting(getApplicationContext(), Lamp_Preferences.FREQUENCY_TEMPERATURE));
                double new_threshold = Double.parseDouble(Lamp.getSetting(getApplicationContext(), Lamp_Preferences.THRESHOLD_TEMPERATURE));
                boolean new_enforce_frequency = (Lamp.getSetting(getApplicationContext(), Lamp_Preferences.FREQUENCY_TEMPERATURE_ENFORCE).equals("true")
                ...
                mSensorManager.registerListener(this, mTemperature, Integer.parseInt(Lamp.getSetting(getApplicationContext(), Lamp_Preferences.FREQUENCY_TEMPERATURE)), sensorHandler);
                LAST_SAVE = System.currentTimeMillis();

                if (Lamp.DEBUG) Log.d(TAG, "Temperature service active...");
            }

            if (Lamp.isStudy(this)) {
                ContentResolver.setIsSyncable(Lamp.getLAMPAccount(this), Temperature_Provider.getAuthority(this), 1);
                ContentResolver.setSyncAutomatically(Lamp.getLAMPAccount(this), Temperature_Provider.getAuthority(this), true);
                long frequency = Long.parseLong(Lamp.getSetting(this, Lamp_Preferences.FREQUENCY_WEBSERVICE)) * 60;
                SyncRequest request = new SyncRequest.Builder()
                        .syncPeriodic(frequency, frequency / 3)
                        .setSyncAdapter(Lamp.getLAMPAccount(this), Temperature_Provider.getAuthority(this))
