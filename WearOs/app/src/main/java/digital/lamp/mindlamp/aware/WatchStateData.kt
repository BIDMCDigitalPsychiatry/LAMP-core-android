...
class WatchStateData constructor(
    awareListener: SensorAwareListener,
    context: Context,
    googleApiClient: GoogleApiClient
) {
    private var receiver: MessageReceiver = MessageReceiver()
    ...
    fun populateValues(vals: CustomMap) {
        ...
        if (vals.dataMap?.getStringArray(Sensor.TYPE_GRAVITY.toString()) != null &&
            ...
            val gravityData =
                GravityData(values?.get(0)?.toDouble(), values?.get(1)?.toDouble(), values?.get(2)?.toDouble())
            val sensorEvenData: SensorEventData = SensorEventData(
                DimensionData(
                    null,
                    null,
        ...
        if (vals.dataMap?.getStringArray(Sensor.TYPE_ROTATION_VECTOR.toString()) != null &&
            ...
            val rotationData =
                RotationData(values?.get(0)?.toDouble(), values?.get(1)?.toDouble(), values?.get(2)
                    ?.toDouble())
            val sensorEvenData: SensorEventData = SensorEventData(
                DimensionData(
                    null,
        ...
        if (vals.dataMap?.getStringArray(Sensor.TYPE_MAGNETIC_FIELD.toString()) != null &&
            ...
            val magnetometerData =
                MagnetData(values?.get(0)?.toDouble(), values?.get(1)?.toDouble(), values?.get(2)?.toDouble())
            val sensorEvenData: SensorEventData = SensorEventData(
                DimensionData(
                    null,
                    null,
        ...
        if (vals.dataMap?.getStringArray(Sensor.TYPE_MOTION_DETECT.toString()) != null &&
            ...
            val motionData =
                MotionData(values?.get(0)?.toDouble(), values?.get(1)?.toDouble(), values?.get(2)?.toDouble())
    ...
    private fun registerReceiver(context: Context) {
        ...
        LocalBroadcastManager.getInstance(context).registerReceiver(receiver, messageFilter)
    }
    ...
    inner class MessageReceiver : BroadcastReceiver() {
        override fun onReceive(
            ...
            awllistener.getWatchData(sensorEventDataList)
            Log.v("Sensor", "Message Received")
