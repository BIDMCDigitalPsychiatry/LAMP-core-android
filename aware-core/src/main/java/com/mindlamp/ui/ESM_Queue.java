...
public class ESM_Queue extends FragmentActivity {
    ...
    private static String TAG = "LAMP::ESM Queue";
    ...
    protected void onCreate(Bundle bundle) {
        ...
        TAG = Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEBUG_TAG).length() > 0 ? Lamp.getSetting(getApplicationContext(), Lamp_Preferences.DEBUG_TAG) : TAG;

        Intent queue_started = new Intent(ESM.ACTION_LAMP_ESM_QUEUE_STARTED);
        sendBroadcast(queue_started);

        IntentFilter filter = new IntentFilter();
        filter.addAction(ESM.ACTION_LAMP_ESM_QUEUE_COMPLETE);
        filter.addAction(ESM.ACTION_LAMP_ESM_DISMISSED);
        filter.addAction(ESM.ACTION_LAMP_ESM_EXPIRED);
        filter.addAction(ESM.ACTION_LAMP_ESM_REPLACED);
    ...
    public class ESM_State extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(ESM.ACTION_LAMP_ESM_QUEUE_COMPLETE)) {
