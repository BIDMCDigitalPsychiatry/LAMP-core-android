    ...
    private synchronized SQLiteDatabase getDatabaseFile() {
        try {
            ...
            if (mContext.getResources().getBoolean(R.bool.internalstorage)) {
                ...
                // sdcard/LAMP/ (shareable, does not delete when uninstalling)
                aware_folder = new File(Environment.getExternalStoragePublicDirectory("LAMP").toString());
            } else {
                if (isEmulator()) {
                    aware_folder = mContext.getFilesDir();
                } else {
                    // sdcard/Android/<app_package_name>/LAMP/ (not shareable, deletes when uninstalling package)
                    aware_folder = new File(ContextCompat.getExternalFilesDirs(mContext, null)[0] + "/LAMP");
                }
            }
            ...
            File file = new File(aware_folder, this.databaseName);
            if(file.exists()){
                database = SQLiteDatabase.openOrCreateDatabase(file.getPath(), this.cursorFactory);
            }
            return database;
