...
public class SSLManager {
    ...
    public static void handleUrl(Context context, String url, boolean block) {
        ...
        if (study_uri.getQuery() != null) {
            ...
            if (crt != null || crt_url != null || crt_sha256 != null)
                if (Lamp.DEBUG)
                    Log.d(Lamp.TAG, "Certificates: Handling URL via query parameters: " + hostname);
            handleCrtParameters(context, hostname, crt, crt_sha256, crt_url);
        } else {
            if (Lamp.getSetting(context, Lamp_Preferences.KEY_STRATEGY).equals("once")) {
                // With "once" management, we only retrieve if we have not gotten cert yet.
                if (Lamp.DEBUG)
                    Log.d(Lamp.TAG, "Certificates: Downloading crt if not present: " + hostname);
                if (!hasCertificate(context, hostname)) {
                    downloadCertificate(context, protocol, hostname, true);
                } else {
                    if (Lamp.DEBUG)
                        Log.d(Lamp.TAG, "Certificates: Already present and key_management=once: " + hostname);
                }
            } else {
                try {
                    if (!hasCertificate(context, hostname)) {
                        if (Lamp.DEBUG) Log.d(Lamp.TAG, "Certificates: Downloading for the first time SSL certificate: " + protocol+"://"+hostname);
                        downloadCertificate(context, protocol, hostname, true);
                    } else {
    ...
    public static X509Certificate retrieveRemoteCertificate(URL url) {
        try {
            ...
            Log.d(Lamp.TAG, "Not an X509Certificate! " + certs[0].getType() + " : " + certs[0].toString());
    ...
    private static void downloadCertificate(Context context, String protocol, String hostname, boolean block) {
        ...
        if (context.getApplicationContext().getResources().getBoolean(R.bool.internalstorage)) {
            ...
            root_folder = new File(Environment.getExternalStoragePublicDirectory("LAMP"), "/credentials/" + hostname); // sdcard/LAMP/ (shareable, does not delete when uninstalling)
        } else {
            root_folder = new File(ContextCompat.getExternalFilesDirs(context, null)[0], "/LAMP/credentials/" + hostname); // sdcard/Android/<app_package_name>/LAMP/ (not shareable, deletes when uninstalling package)
        }
        root_folder.mkdirs();
        ...
        try {
            ...
            Log.d(Lamp.TAG, "SSL error: " + e.getMessage());
        }
    }
    ...
    private static void handleCrtParameters(Context context, String hostname, String crt, String crt_sha256, String crt_url) {
        if (Lamp.DEBUG) {
            Log.d(Lamp.TAG, "handleCrtParameters");
            Log.d(Lamp.TAG, "crt=" + crt);
            Log.d(Lamp.TAG, "crt_url=" + crt_url);
            Log.d(Lamp.TAG, "crt_sha256=" + crt_sha256);
        }
        ...
        if (crt != null) {
            ...
            try {
                ...
                crt = sb.toString();
                if (Lamp.DEBUG) {
                    Log.d(Lamp.TAG, "Downloaded crt=" + crt);
                }
            } catch (IOException e) {
                Log.e(Lamp.TAG, "Certificates: Can not download crt: " + crt_url);
                // TODO: error handling
                return;
            }
        } else {
            // TODO: error handling
            Log.e(Lamp.TAG, "Certificates: Both crt and crt_url are null: ");
            return;
        }
        ...
        if (crt_sha256 != null) {
            ...
            if (!actual_hash.equals(crt_sha256)) {
                Log.e(Lamp.TAG, "Invalid certificate hash: " + crt_sha256 + "!=" + actual_hash);
                return;
            }
        }
    ...
    public static boolean hasCertificate(Context context, String hostname) {
        ...
        if (context.getResources().getBoolean(R.bool.internalstorage)) {
            ...
            root_folder = new File(Environment.getExternalStoragePublicDirectory("LAMP") + "/credentials"); // sdcard/LAMP/ (shareable, does not delete when uninstalling)
        } else {
            root_folder = new File(ContextCompat.getExternalFilesDirs(context, null)[0] + "/LAMP/credentials"); // sdcard/Android/<app_package_name>/LAMP/ (not shareable, deletes when uninstalling package)
        }
        ...
        try {
            ...
            Log.d(Lamp.TAG, "Error getting HTTPS SSL certificate: " + e.getMessage());
            return false;
        }
    }
    ...
    private static void setCertificate(Context context, String hostname, String cert_data) {
        ...
        try {
            ...
            cert_f.close();
            Log.d(Lamp.TAG, "Set certificate for " + hostname);
        } catch (java.io.IOException e) {
            Log.d(Lamp.TAG, "Can not write certificate: " + cert_file);
            e.printStackTrace();
        }
    }
