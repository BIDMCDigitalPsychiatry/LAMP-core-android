...
import com.mindlamp.providers.ESM_Provider;
import com.mindlamp.providers.ESM_Provider.ESM_Data;
import com.mindlamp.ui.ESM_Queue;
import com.mindlamp.ui.esms.ESMFactory;
import com.mindlamp.ui.esms.ESM_Question;
...
public class ESM extends Aware_Sensor {
    ...
    /**
     * Logging tag (default = "LAMP::ESM")
     */
    private static String TAG = "LAMP::ESM";

    /**
     * Received event: queue the specified ESM
     * Extras: (JSONArray as String) esm
     */
    public static final String ACTION_LAMP_QUEUE_ESM = "ACTION_LAMP_QUEUE_ESM";

    /**
     * Received event: try the specified ESM
     * Extras: (JSONArray as String) esm
     */
    public static final String ACTION_LAMP_TRY_ESM = "ACTION_LAMP_TRY_ESM";

    /**
     * Broadcasted event: the user has answered one answer from ESM queue
     */
    public static final String ACTION_LAMP_ESM_ANSWERED = "ACTION_LAMP_ESM_ANSWERED";

    /**
     * Broadcasted event: the user has dismissed one answer from ESM queue
     */
    public static final String ACTION_LAMP_ESM_DISMISSED = "ACTION_LAMP_ESM_DISMISSED";

    /**
     * Broadcasted event: the user did not answer the ESM on time from ESM queue
     */
    public static final String ACTION_LAMP_ESM_EXPIRED = "ACTION_LAMP_ESM_EXPIRED";

    /**
     * Broadcasted event: the ESM got replaced by another ESM
     */
    public static final String ACTION_LAMP_ESM_REPLACED = "ACTION_LAMP_ESM_REPLACED";

    /**
     * Broadcasted event: the user has finished answering the ESM queue
     */
    public static final String ACTION_LAMP_ESM_QUEUE_COMPLETE = "ACTION_LAMP_ESM_QUEUE_COMPLETE";

    /**
     * Broadcasted event: the user has started answering the ESM queue
     */
    public static final String ACTION_LAMP_ESM_QUEUE_STARTED = "ACTION_LAMP_ESM_QUEUE_STARTED";
     ...
     * Example: [{"esm":{"esm_type":7,"esm_title":"Date and time","esm_instructions":"When did this happen?","esm_submit":"OK","esm_trigger":"LAMP Test"}}]
     ...
     * Example: [{"esm":{"esm_type":8,"esm_title":"PAM","esm_instructions":"Select what best illustrates your mood","esm_submit":"OK","esm_trigger":"LAMP Test"}}]
     */
    ...
    /**
     * Extra for ACTION_LAMP_ESM_ANSWERED as String
     */
    ...
    public void onCreate() {
        ...
        if (Lamp.DEBUG) Log.d(TAG, "ESM service created!");

        IntentFilter filter = new IntentFilter();
        filter.addAction(ACTION_LAMP_TRY_ESM);
        filter.addAction(ACTION_LAMP_QUEUE_ESM);
        filter.addAction(ACTION_LAMP_ESM_ANSWERED);
        filter.addAction(ACTION_LAMP_ESM_DISMISSED);
        filter.addAction(ACTION_LAMP_ESM_EXPIRED);
        filter.addAction(ACTION_LAMP_ESM_REPLACED);
    ...
    public void onDestroy() {
        ...
        ContentResolver.setSyncAutomatically(Lamp.getLAMPAccount(this), ESM_Provider.getAuthority(this), false);
        ...
        if (Lamp.DEBUG) Log.d(TAG, "ESM service terminated...");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (PERMISSIONS_OK) {
            ...
            Lamp.setSetting(this, Lamp_Preferences.STATUS_ESM, true);

            if (Lamp.getSetting(getApplicationContext(), Lamp_Preferences.STATUS_ESM).equals("true")) {
            ...
            if (Lamp.isStudy(this)) {
                ContentResolver.setIsSyncable(Lamp.getLAMPAccount(this), ESM_Provider.getAuthority(this), 1);
                ContentResolver.setSyncAutomatically(Lamp.getLAMPAccount(this), ESM_Provider.getAuthority(this), true);
                long frequency = Long.parseLong(Lamp.getSetting(this, Lamp_Preferences.FREQUENCY_WEBSERVICE)) * 60;
                SyncRequest request = new SyncRequest.Builder()
                        .syncPeriodic(frequency, frequency / 3)
                        .setSyncAdapter(Lamp.getLAMPAccount(this), ESM_Provider.getAuthority(this))
    ...
    public static void queueESM(Context context, String queue, boolean isTrial) {
        try {
            ...
            for (int i = 0; i < esms.length(); i++) {
                ...
                rowData.put(ESM_Data.DEVICE_ID, Lamp.getSetting(context, Lamp_Preferences.DEVICE_ID));
                ...
                try {
                    ...
                    if (Lamp.DEBUG) Log.d(TAG, "ESM: " + rowData.toString());
        ...
        protected Void doInBackground(Void... params) {
            if (mRetries == 0) {
                ...
                while (mRetries > 0) {
                    ...
                    if (Lamp.DEBUG) Log.d(TAG, "Retrying ESM: " + mRetries);
            ...
            //Expire the queue
            Intent expired = new Intent(ESM.ACTION_LAMP_ESM_EXPIRED);
     ...
     * - ACTION_LAMP_QUEUE_ESM
     * - ACTION_LAMP_ESM_ANSWERED
     * - ACTION_LAMP_ESM_DISMISSED
     * - ACTION_LAMP_ESM_EXPIRED
     *
     * @author df
     */
    public static class ESMMonitor extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (!Lamp.getSetting(context, Lamp_Preferences.STATUS_ESM).equals("true")) return;
            ...
            if (context.getPackageName().equalsIgnoreCase("com.aware.phone") || context.getApplicationContext().getResources().getBoolean(R.bool.standalone)) {
                ...
                if (intent.getAction().equals(ESM.ACTION_LAMP_ESM_ANSWERED)) {
                    ...
                    if (ESM_Queue.getQueueSize(context) > 0) {
                        ...
                        if (Lamp.DEBUG) Log.d(TAG, "ESM Queue is done!");
                        Intent esm_done = new Intent(ESM.ACTION_LAMP_ESM_QUEUE_COMPLETE);
                        context.sendBroadcast(esm_done);
                    }
                }

                if (intent.getAction().equals(ESM.ACTION_LAMP_ESM_DISMISSED)) {
                    ...
                    if (Lamp.DEBUG) Log.d(TAG, "Rest of ESM Queue is dismissed!");

                    Intent esm_done = new Intent(ESM.ACTION_LAMP_ESM_QUEUE_COMPLETE);
                    context.sendBroadcast(esm_done);
                }

                if (intent.getAction().equals(ESM.ACTION_LAMP_ESM_EXPIRED)) {
                    Cursor esm = context.getContentResolver().query(ESM_Data.CONTENT_URI, null, ESM_Data.STATUS + " IN (" + ESM.STATUS_NEW + "," + ESM.STATUS_VISIBLE + ")", null, null);
                    if (esm != null && esm.moveToFirst()) {
                        do {
                            ContentValues rowData = new ContentValues();
                    ...
                    if (Lamp.DEBUG) Log.d(TAG, "Rest of ESM Queue is expired!");
                    Intent esm_done = new Intent(ESM.ACTION_LAMP_ESM_QUEUE_COMPLETE);
                    context.sendBroadcast(esm_done);
                }
            }
            ...
            if (last_esm != null && last_esm.moveToFirst()) {
                ...
                for (int i = 0; i < flows.length(); i++) {
                    ...
                    if (flowAnswer.equals(current_answer)) {
                        if (Lamp.DEBUG) Log.d(ESM.TAG, "Following next question: " + nextESM);
