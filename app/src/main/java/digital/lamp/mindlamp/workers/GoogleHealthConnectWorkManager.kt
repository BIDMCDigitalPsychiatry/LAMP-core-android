...
class GoogleHealthConnectWorkManager(private val context: Context, parameters: WorkerParameters) :
    ...
    var oSensorSpecList = arrayListOf<SensorSpecs>()
    ...
    override suspend fun doWork(): Result {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            oSensorSpecList = oSensorDao.getSensorsList() as ArrayList<SensorSpecs>
            Thread.sleep(5000)
            if (oSensorSpecList?.isNotEmpty() == true) {
                ...
                readBloodSugar()
                readRespiratoryRate()
                getGoogleHealthConnect(sensorEventDataList)
    ...
    private suspend fun readBloodSugar() {
        try {
            ...
            if (sensorList.isNotEmpty()) {
                getGoogleHealthConnect(sensorList)
            }
        } catch (e: Exception) {
    ...
    private fun getGoogleHealthConnect(sensorEventData: java.util.ArrayList<SensorEvent>) {
        val gson = GsonBuilder().serializeNulls().create()
        LampLog.e("Google Health connect 1: ${gson.toJson(sensorEventData)}")

        val oAnalyticsList: java.util.ArrayList<Analytics> = arrayListOf()
        GlobalScope.async {
