 ...
 */
class Gravity constructor(awareListener: AwareGravityListener, context: Context, sensorname:String) {
    init {
        try {
            //gravity settings
            Aware.setSetting(
            ...
            Gravity.setSensorObserver {
                 x = it.getAsDouble(Gravity_Provider.Gravity_Data.VALUES_0)?: 0.0
                 y = it.getAsDouble(Gravity_Provider.Gravity_Data.VALUES_1)?: 0.0
                 z = it.getAsDouble(Gravity_Provider.Gravity_Data.VALUES_2)?: 0.0
                if (it != null) {
                    val dimensionData =
                        DimensionData(
                            x,
